...
makefolder("kingltnFarmer")
if not isfile("kingltnFarmer/Fruits.txt") then writefile("kingltnFarmer/Fruits.txt", "false") end

local settings = getgenv().KingLTNFarmerConfig
...
fileName = string.format("kingltnFarmer/%s_servers.json", tostring(PlaceId))
ServerHopData = { 
    CheckedServers = {},
    LastTimeHop = nil,
    CreatedAt = os.time()
...
if (math.ceil(Library.FruitCmds.Get(game.Players.LocalPlayer, Library.FruitCmds.Directory.Orange)) <= settings.Fruits.StartFarmAt) or (readfile("kingltnFarmer/Fruits.txt") == "true") then
    writefile("kingltnFarmer/Fruits.txt", "true")

    if math.ceil(Library.FruitCmds.Get(game.Players.LocalPlayer, Library.FruitCmds.Directory.Orange)) >= settings.Fruits.Max then
        writefile("kingltnFarmer/Fruits.txt", "false")
        ...
        local embed = {
            {
                ...
                ["footer"] = {
                    ["text"] = "Script Farmer • Mystic Farmer"..webhook_user,
                    ["icon_url"] = "https://media.discordapp.net/attachments/1108132899431137300/1109902689996910622/gpfp.png?width=675&height=675"
                },
                ["timestamp"] = DateTime.now():ToIsoDate()
            }
